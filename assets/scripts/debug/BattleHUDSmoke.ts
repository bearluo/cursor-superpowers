...
export class BattleHUDSmoke extends Component {
  start(): void {
    ...
    EventBus.emit(EVENTS.ReactionResolved, { reactionId: 'OverheatDischarge', damage: 24, affectedCount: 3 });
    ...
    EventBus.emit(EVENTS.XpProgressChanged, { xpCurrent: 7, xpNeed: 18, rewardLevel: 2 });
    EventBus.emit(EVENTS.EnemyAliveChanged, { alive: 9, cap: 14, wave: 2, zone: 1 });
    EventBus.emit(EVENTS.ReinforcementSpawned, { count: 2, alive: 11, cap: 14, wave: 2, zone: 1 });
